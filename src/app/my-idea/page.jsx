...
import { useState, useEffect } from 'react';
import { authClient } from '@/lib/auth-client';
import EditIdeaModal from '@/components/myideamodal'; // adjust path as needed
...
export default function MyIdeasPage() {
  ...
  // ── Modal state ──────────────────────────────────────────────────────────────
  const [editingId, setEditingId] = useState(null); // null = closed
  ...
  if (isPending) return <div className="p-12 text-center">Loading session…</div>;
  ...
  return (
    <main className="p-6 max-w-4xl mx-auto min-h-screen bg-white dark:bg-gray-900">
          ...
          {ideas.map((idea) => (
            ...
            >
              ...
              <div className="p-6 flex-1">
                ...
                <div className="flex justify-between mb-4">
                  <div className="flex gap-2">
                    <span className="px-3 py-1 text-xs bg-blue-100 dark:bg-blue-900 text-white rounded-full">
                      {idea.category}
                    </span>
                    <span className="px-3 py-1 text-xs bg-emerald-100 dark:bg-emerald-900 text-white rounded-full">
                      {idea.status}
                    </span>
                  </div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="px-4 py-1.5 text-sm border rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition"
                    >
                      Edit
                    </button>
                    <button
                      onClick={() => handleDelete(idea._id)}
                      className="px-4 py-1.5 text-sm text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg transition"
                    >
                      Delete
                    </button>
                  </div>
                ...
                <div className="text-sm flex justify-between border-t pt-4 text-gray-500">
                  <span>Funding: <strong>{idea.funding}</strong></span>
      ...
      {/* ✅ Modal — only renders when an idea is being edited */}
      {editingId && (
        <EditIdeaModal
          ideaId={editingId}
          onClose={() => setEditingId(null)}
          onSuccess={fetchMyIdeas}        // refreshes list silently
        />
      )}
    </main>
  );
